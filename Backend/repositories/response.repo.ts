...
import { FormResponse } from "../types/index.js";

export function mapResponse(row: any): FormResponse | null {
    if (!row) return null;
    return {
        _id: row.id,
        id: row.id,
...
export async function insertManyResponses(docs: any[]): Promise<void> {
    ...
    const submitted = docs.map((d) => (d.submittedAt ? new Date(d.submittedAt).toISOString() : new Date().toISOString()));
...
export async function recentByForms(formIds: string[], limit: number = 300): Promise<FormResponse[]> {
    ...
    return rows.map((r) => mapResponse(r)!);
}

export async function listResponsesByForm(formId: string): Promise<FormResponse[]> {
    const { rows } = await query(
...
export async function findResponseById(id: string): Promise<FormResponse | null> {
...
export async function timelineByForms(formIds: string[]): Promise<{ date: string; count: number }[]> {
    if (!formIds.length) return [];
    const { rows } = await query(
...
export async function avgCompletionByForms(formIds: string[]): Promise<number> {
...
export async function heatmapByForms(formIds: string[]): Promise<{ day: number; hour: number; count: number }[]> {
    if (!formIds.length) return [];
    const { rows } = await query(
...
export async function devicesByForms(formIds: string[]): Promise<{ label: string; value: number }[]> {
    if (!formIds.length) return [];
    const { rows } = await query(
        `SELECT CASE
