...
import { FieldType, Form, Question, QuestionOption, QuestionValidation } from "../types/index.js";

const FIELD_TYPE_LIST = FIELD_TYPES.join(", ");

export async function generateForm(prompt?: string): Promise<Partial<Form>> {
...
export async function improveQuestion({
    ...
    label: string;
    type?: string;
}): Promise<{ label: string; helpText: string; placeholder: string }> {
...
export async function summarizeForm(form: Partial<Form>): Promise<{
    purpose: string;
    audience: string;
...
function normalizeForm(data: any): Partial<Form> {
    ...
    return {
        ...
        questions: questions.map((q: any, index: number) => normalizeQuestion(q, index)),
    };
}

function normalizeQuestion(q: any, index: number): Question {
    const type: FieldType = FIELD_TYPES.includes(q?.type) ? q.type : "short_text";
