...
import { Form, FormResponse, Question } from "../types/index.js";

export async function submitResponse(
    slug: string,
    { answers = [], completionTime = 0, meta = {} }: { answers?: any[]; completionTime?: number; meta?: any }
...
function validateAnswer(q: Question, value: any): void {
    ...
    if (OPTION_FIELD_TYPES.includes(q.type) && q.options && q.options.length) {
...
export async function deleteResponse(responseId: string, userId: string): Promise<void> {
...
export async function exportResponses(formId: string, userId: string): Promise<{ form: Form; responses: FormResponse[] }> {
