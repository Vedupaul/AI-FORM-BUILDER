...
import { STATIC_FIELD_TYPES } from "./constants.js";
import { FieldType, Form } from "../types/index.js";

const STATIC = new Set(STATIC_FIELD_TYPES);
...
const q = (type: FieldType, label: string, extra: any = {}) => ({
    id: nanoid(10),
    type,
    label,
...
const FIRST = ["Alex", "Maya", "Liam", "Sofia", "Noah", "Emma", "Ravi", "Chloe", "Omar", "Ava", "Ben", "Zoe", "Luca", "Theo", "Nora", "Kai", "Mia", "Sam", "Aria", "Leo"];
const LAST = ["Carter", "Singh", "Nguyen", "Garcia", "Brooks", "Khan", "Russo", "Miller", "Silva", "Patel", "Cohen", "Lopez", "Tanaka", "Novak", "Thomas"];
const COMMENTS = [
    "Good experience, super smooth!",
    "Unclear UI, a bit confusing at first.",
...
const CITIES = ["123 Market St, San Francisco, CA", "44 King St W, Toronto, ON", "10 Downing St, London", "MG Road, Bangalore", "22 Rue Cler, Paris", "8 Bondi Rd, Sydney"];
const USER_AGENTS = [
    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/124.0 Safari/537.36",
    "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7) Version/17.4 Safari/605.1",
...
function answerFor(qq: any, who: any): any {
    ...
    switch (qq.type) {
        ...
        case "dropdown":
        case "radio":
            return qq.options.length ? (pick(qq.options as any[]) as any).label : "";
        case "checkbox":
            return qq.options.length ? sample(qq.options, rand(1, Math.min(3, qq.options.length))).map((o: any) => o.label) : [];
        case "file":
...
function buildResponse(form: Form, dayOffset: number) {
    ...
    return {
        ...
        meta: { userAgent: pick(USER_AGENTS), ip: `${rand(1, 255)}.${rand(0, 255)}.${rand(0, 255)}.${rand(1, 255)}` },
...
const FORMS = [
    ...
    },
    {
        ...
        questions: [
            ...
            q("rating", "How valuable is this to you?"),
