...
export const getResponses = asyncHandler(async (req, res) => {
    const responses = await responseService.listResponses(req.params.id, req.user._id, {
...
export const exportResponses = asyncHandler(async (req, res) => {
    const { form, responses } = await responseService.exportResponses(req.params.id, req.user._id);
